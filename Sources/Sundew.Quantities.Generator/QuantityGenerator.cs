namespace Sundew.Quantities.Generator
{
    ...
    public class QuantityGenerator : IModelGenerator<QuantityDefinition>
    {
        public string GetFileName(IGeneratorSettings generatorSettings, QuantityDefinition modelDefinition)
        {
            return $"{modelDefinition.Name}{generatorSettings.OutputFileNameSuffix}{generatorSettings.OutputFileExtension}";
        }

        public GeneratorString Generate(string classFileName, IGeneratorSettings codeGeneratorSettings, Project project,
            QuantityDefinition modelDefinition)
        {
            return
$@"
// --------------------------------------------------------------------------------------------------------------------
...
{{
{GetUsings(codeGeneratorSettings.UseGlobalUsings | modelDefinition.UseGlobalUsings, 4, codeGeneratorSettings.Usings, modelDefinition.Usings)}

    [GeneratedCode(""Sundew.Quantities.Generator"", ""1.0.0.0"")]
    public partial struct {modelDefinition.Name} : IQuantity<{modelDefinition.Name}, {modelDefinition.UnitSelector}>
    {{
        private readonly double value;

{GetConstructors(modelDefinition)}

        double IQuantity.Value => this.value;

        public IUnit Unit {{ get; }}

{GetStandardOperations(modelDefinition)}

{GetConversionOperations(modelDefinition)}

{GetEqualityOperations(modelDefinition)}

{GetToStringOperations(modelDefinition)}

        public {modelDefinition.Name} CreateQuantity(double value, IUnit unit)
        {{
            return new {modelDefinition.Name}(value, unit);
        }}
        ...
        {modelDefinition.UnitSelector} IQuantity<{modelDefinition.Name}, {modelDefinition.UnitSelector}>.CreateUnitSelector()
        {{
            return new {modelDefinition.UnitSelector}();
        }}
    }}
}}
";
