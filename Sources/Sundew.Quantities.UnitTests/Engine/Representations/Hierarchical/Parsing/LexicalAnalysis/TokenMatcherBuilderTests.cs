namespace Sundew.Quantities.UnitTests.Engine.Representations.Hierarchical.Parsing.LexicalAnalysis
{
    ...
    public class TokenMatcherBuilderTests
    {
        ...
        [Theory]
        [InlineData(new[] { "Y", "Z", "P", "T", "G", "M", "k", "h", "da", "d", "c", "m", "µ", "n", "p", "f", "a", "z", "y" }, true, @"^(?<ID>Y|Z|P|T|G|M|k|h|da|d|c|m|µ|n|p|f|a|z|y)?.+$")]
        [InlineData(new[] { "mi", "m", "in", "f", "yd", "nmi", "y", "mon", "w", "d", "h", "min", "s", "A", "V", "R", "K", "C", "F", "mol", "cd" }, false, @"^(?<ID>mi|m|in|f|yd|nmi|y|mon|w|d|h|min|s|A|V|R|K|C|F|mol|cd)$")]
        public void Build_ThenResultStringShouldBeExpectedRegex(string[] validTokens, bool areOptional, string expectedRegex)
        {
